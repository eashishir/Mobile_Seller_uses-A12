...
const AllUser = () => {
    ...
    const { data: users = [], refetch } = useQuery({
        queryKey: ['users'],
        queryFn: async () => {
            const res = await fetch('https://my-assignment12-server.vercel.app/users');
    ...
    const handleMakeAdmin = id => {
        fetch(`https://my-assignment12-server.vercel.app/users/admin/${id}`, {
            method: "PUT",
            headers: {
    ...
    const handleDeleteUser = user => {
        fetch(`https://my-assignment12-server.vercel.app/users/${user._id}`, {
            method: 'DELETE',
            headers: {
                authorization: `bearer ${localStorage.getItem('accessToken')}`
            }
